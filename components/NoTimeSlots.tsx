...
export default function NoTimeSlots({
    ...
    return (
      <Card className="max-w-md mx-auto border-4 border-blue-500/10 shadow-2xl transition delay-150 duration-500 ease-in-out hover:-translate-y-1 hover:scale-125">
        ...
        <CardContent>
          {calendarUser.fullName} no tiene disponibilidad en este momento. Vuelve más tarde o elige un evento más corto.
        </CardContent>
        <CardFooter>
          <Button 
          className="cursor-pointer hover:scale-105 bg-blue-400 hover:bg-blue-600"
          asChild>
            <Link href={`/book/${calendarUser.id}`}>Elegir otro evento</Link>
          </Button>
        </CardFooter>
      </Card>
    )
